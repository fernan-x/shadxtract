...
describe("USECASE: UpdateWorkflowUseCase", () => {
  ...
  it("should throw error when workflow is not found", async () => {
    await expect(useCase.execute("unknown", "Test Workflow")).rejects.toThrow(
      "Workflow not found",
    );
  });

  it("should throw error when workflow is not in draft status", async () => {
    await expect(useCase.execute("1", "Test Workflow")).rejects.toThrow(
      "Cannot update workflow with status other than draft",
    );
  });
